...
const ImprovedChatComponent = () => {
  ...
  return (
    <div className="flex flex-col min-h-[100svh] bg-gradient-to-br from-gray-50 to-gray-100 rounded-none md:rounded-2xl shadow-none md:shadow-xl overflow-hidden border border-gray-200">
      {/* Header */}
      <header className="bg-gradient-to-r from-indigo-600 via-purple-600 to-blue-600 text-white p-3 sm:p-4 md:p-5">
        <div className="flex flex-col xs:flex-row justify-between items-center gap-2 sm:gap-3 md:gap-4">
          <div className="flex-1 flex items-center gap-2 sm:gap-3 md:gap-4 min-w-0">
            ...
            <div className="min-w-0">
              <h1 className="text-sm sm:text-base md:text-lg lg:text-xl font-bold truncate">RAG Todo Chatbot</h1>
              <p className="text-indigo-200 text-xs sm:text-sm md:text-base hidden xs:block">Manage your tasks with natural language</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="bg-white/20 hover:bg-white/30 text-white px-3 py-1.5 sm:px-4 sm:py-2 md:px-5 md:py-2.5 rounded-lg transition-all duration-300 flex items-center gap-2 whitespace-nowrap shadow-md hover:shadow-lg text-xs sm:text-sm md:text-base"
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 sm:h-5 sm:w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
              </svg>
              <span className="truncate max-w-[60px] xs:max-w-[80px] sm:max-w-[100px]">{showTaskPanel ? 'Hide Tasks' : 'Show Tasks'}</span>
            </button>

            <button
              className="md:hidden text-white p-2 rounded-lg bg-white/20 hover:bg-white/30"
      ...
      <div className="flex flex-1 overflow-hidden flex-col md:flex-row">
              ...
              {tasks.length === 0 ? (
                ...
                <div className="space-y-3 sm:space-y-4 flex-1 overflow-y-auto">
                  {tasks.map((task) => (
                    <div
                      key={task.id}
                      className={`p-3 sm:p-4 rounded-xl sm:rounded-2xl border transition-all duration-300 transform hover:scale-[1.01] ${
                        task.completed
        ...
        {/* Desktop Task Panel */}
        <div className={`${showTaskPanel ? 'hidden md:flex' : 'hidden'} md:w-1/3 lg:w-1/4 xl:w-1/5 bg-gradient-to-b from-white to-gray-50 p-4 sm:p-5 overflow-y-auto max-h-[calc(100svh-4rem)] md:max-h-none border-r border-gray-200 flex-col`}>
          <div className="w-full">
            <div className="flex justify-between items-center mb-3 sm:mb-5 pb-2 sm:pb-3 border-b border-gray-200">
              ...
              <button
                onClick={toggleTaskPanel}
                className="text-gray-500 hover:text-gray-700 md:hidden p-1 rounded-full hover:bg-gray-100 transition-colors"
              >
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                </svg>
        ...
        <div className={`${showTaskPanel ? 'md:w-2/3 lg:w-3/4 xl:w-4/5' : 'w-full'} flex flex-col bg-gradient-to-b from-white to-gray-50 flex-1`}>
          ...
          <div className="border-b border-gray-200 p-3 sm:p-4 bg-gradient-to-r from-blue-50/50 to-indigo-50/50 backdrop-blur-sm">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
              <div className="text-gray-600 break-all max-w-[70%] flex items-center gap-2">
                ...
                {conversationId ? (
                  <span className="text-sm sm:text-base">Conv: {conversationId.substring(0, 8)}...</span>
                ) : (
                  <span className="text-sm sm:text-base">New conversation</span>
                )}
              </div>
              <div className="text-gray-600 break-all max-w-[70%] flex items-center gap-2">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 sm:h-5 sm:w-5 text-indigo-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                </svg>
                {userId && <span className="text-sm sm:text-base">User: {userId.substring(0, 8)}...</span>}
              </div>
            </div>
          </div>

          {/* Messages Area */}
          <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-4" style={{ maxHeight: 'calc(100svh - 120px)' }}>
            {messages.length === 0 ? (
              <div className="text-center text-gray-600 mt-6 sm:mt-10 px-2 sm:px-4 flex flex-col items-center">
                ...
                <h3 className="text-lg sm:text-xl md:text-2xl font-bold text-gray-800 mb-2 sm:mb-3">Welcome to the RAG Todo Chatbot!</h3>
                <p className="mb-4 sm:mb-6 text-gray-600 text-sm sm:text-base">How can I help you manage your tasks today?</p>
                <div className="grid grid-cols-1 gap-3 sm:gap-4 max-w-xs sm:max-w-md md:max-w-lg mx-auto w-full">
                  <div className="bg-gradient-to-br from-indigo-50 to-blue-50 rounded-lg sm:rounded-xl p-3 sm:p-4 text-left border border-indigo-100 shadow-sm text-xs sm:text-sm md:text-base transition-all duration-300 hover:shadow-md">
              ...
              <div className="space-y-4">
                {messages.map((msg, index) => (
                  ...
                  >
                    <div
                      className={`max-w-[90%] rounded-2xl sm:rounded-3xl p-4 sm:p-5 transition-all duration-300 transform hover:scale-[1.02] ${
                        msg.role === 'user'
                          ? 'bg-gradient-to-br from-indigo-500 to-purple-600 text-white rounded-br-none shadow-lg hover:shadow-xl'
                          : 'bg-gradient-to-br from-gray-100 to-gray-200 text-gray-800 rounded-bl-none shadow hover:shadow-md'
                      }`}
                    >
                      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 mb-2">
                        <div className="flex items-center gap-2">
                          ...
                          <span className={`text-sm sm:text-base font-medium ${msg.role === 'user' ? 'text-indigo-100' : 'text-gray-600'}`}>
                            {msg.role === 'user' ? 'You' : 'Assistant'}
                          </span>
                        </div>
                        <span className={`text-xs sm:text-sm ${msg.role === 'user' ? 'text-indigo-200' : 'text-gray-500'}`}>
                          {msg.timestamp.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                        </span>
                      </div>
                      <div className={`prose prose-sm max-w-none text-sm sm:text-base ${msg.role === 'user' ? 'text-white' : 'text-gray-800'}`}>
                        {msg.content.split('\n').map((line, i) => (
                      ...
                      {msg.tool_calls && msg.tool_calls.length > 0 && (
                        <div className="mt-3 pt-3 border-t border-white/30">
                          ...
                          {msg.tool_calls.map((tool, idx) => (
                            <div key={idx} className="text-sm opacity-90 break-all bg-black/5 p-2 rounded-lg mb-1 last:mb-0">
                              <span className="font-mono bg-white/20 px-2 py-0.5 rounded text-xs">{tool.tool_name}</span>
          ...
          <div className="p-4 sm:p-5 border-t border-gray-200 bg-gradient-to-b from-gray-50 to-white sticky bottom-0">
            <div className="flex flex-col gap-4">
              <div className="flex flex-col gap-4">
                ...
                <button
                  ...
                  className="bg-gradient-to-r from-indigo-600 to-purple-600 text-white px-5 py-3.5 sm:px-6 sm:py-4 rounded-xl sm:rounded-2xl hover:from-indigo-700 hover:to-purple-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 disabled:opacity-50 flex items-center justify-center w-full sm:w-auto sm:self-end shadow-md hover:shadow-lg transition-all duration-300 text-sm sm:text-base"
                >
                  {isLoading ? (
            ...
            <div className="mt-3 sm:mt-4 text-xs sm:text-sm text-gray-500 flex flex-col sm:flex-row justify-between items-center gap-3">
              <div className="flex flex-wrap gap-3 justify-center sm:justify-start">
                {userId && <span className="break-all max-w-[100px] sm:max-w-none flex items-center gap-1"><svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-indigo-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg> User: {userId.substring(0, 8)}...</span>}
                {conversationId && <span className="break-all max-w-[100px] sm:max-w-none flex items-center gap-1"><svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-blue-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z" /></svg> Conv: {conversationId.substring(0, 8)}...</span>}
              </div>
...
export default ImprovedChatComponent;